...
def process_reaction(input_path, reaction_name, output_dir='./output'):
    ...
    df_raw = df_raw[~df_raw['Source'].str.contains('lmp_bks')]  # 排除BKS数据
    ...
    # 设置字体属性（修正后的写法）
    title_font = {'family': plt.rcParams['font.family'], 'size': 20, 'weight': 'bold'}
    ...
    plt.ylabel(r'Energy /kcal$\cdot$mol$^{-1}$', **axis_font)

    
    # 修正刻度字体设置
    plt.xticks(range(1, 8), fontsize=25)
    ...
    plt.legend(
        ...
        loc='upper left',
        frameon=True,
        prop={'family': plt.rcParams['font.family'], 'size': 25}
    )
    plt.grid(False)
