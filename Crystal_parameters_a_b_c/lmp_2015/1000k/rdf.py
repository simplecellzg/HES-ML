...
# 修改字体设置，避免Times New Roman找不到的问题
# 尝试使用系统字体
try:
...
# 全局字体放大设置（放大2倍）
plt.rcParams['font.size'] = 24  # 基础字体大小从12改为24
plt.rcParams['axes.titlesize'] = 32  # 修改3：增大标题字体
plt.rcParams['axes.labelsize'] = 28  # 修改3：增大轴标签字体
plt.rcParams['xtick.labelsize'] = 20  # x轴刻度字体大小
plt.rcParams['ytick.labelsize'] = 20  # y轴刻度字体大小
plt.rcParams['legend.fontsize'] = 20  # 图例字体大小

# 创建输出文件夹
output_dir = 'lammps_analysis_results'
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

def read_lammps_trajectory(dump_file, max_frames=None):
    ...
    while i < len(lines):
        if lines[i].startswith('ITEM: TIMESTEP'):
            ...
            if lines[i].startswith('ITEM: ATOMS'):
                # 解析列标题
                headers = lines[i].split()[2:]  # 去掉 "ITEM: ATOMS"
                ...
                for j in range(n_atoms):
                    ...
                    symbol = parts[element_idx]
                    x = float(parts[x_idx])
                    y = float(parts[y_idx])
                    z = float(parts[z_idx])
                    
                    atom_data.append((atom_id, symbol, x, y, z))
                    
                # 按原子ID排序
                atom_data.sort(key=lambda x: x[0])
                
                # 提取排序后的数据
                for atom_id, symbol, x, y, z in atom_data:
                    symbols.append(symbol)
                    positions.append([x, y, z])
                
                # 创建ASE Atoms对象
                atoms = Atoms(symbols=symbols, positions=positions)
                atoms.set_cell([[lx, 0, 0], [0, ly, 0], [0, 0, lz]])
                atoms.set_pbc(True)
...
def plot_and_save_rdf(rdf_data, r_max, output_prefix='rdf'):
    """Plot and save RDF with peak annotations"""
    ...
    if 'O-O' in peak_info:
        ...
        plt.text(peak_r - 0.1, peak_g + 0.1,  # 修改：放到左边
                f'O-O\nr={peak_r:.2f} Å\ng(r)={peak_g:.2f}', 
    ...
    if 'Si-O' in peak_info:
        ...
        plt.text(peak_r - 0.15, text_y,  # 修改：向下调整
                f'Si-O\nr={peak_r:.2f} Å\ng(r)={peak_g:.2f}', 
...
def main():
    # 文件路径 - 修改为LAMMPS轨迹文件
    dump_file = 'SiO2.lammpstrj'  # 请替换为您的LAMMPS轨迹文件路径
    
    print("Reading LAMMPS trajectory file...")
    frames = read_lammps_trajectory(dump_file)
    ...
    # 选择最后的50帧
    n_frames_to_use = min(50, len(frames))  # 使用最后50帧
    if n_frames_to_use < 50:
        print(f"Warning: Only {len(frames)} frames available, less than requested 50 frames")
    ...
    with open(os.path.join(output_dir, 'comprehensive_analysis_report.txt'), 'w') as f:
        f.write("SiO2 System Structure Analysis Report (LAMMPS)\n")
        f.write("="*50 + "\n\n")
    ...
    print("\nAnalysis complete!")
    print("Generated files in lammps_analysis_results folder:")
